...
PLAN_PATH = os.path.join(BASE_DIR, "iws2", "plan.txt")
...
def load_yaml(path):
    ...
    try:
        ...
        st.error(f"Ошибка чтения YAML {path}: {e}")
        return {}
...
if uploaded_csv:
    try:
        csv_content = pd.read_csv(uploaded_csv).to_csv(index=False)
    except Exception:
        st.sidebar.error("Ошибка CSV")

if uploaded_txt:
    try:
        txt_content = uploaded_txt.read().decode("utf-8")
    except Exception:
        st.sidebar.error("Ошибка TXT")
...
with col1:
    ...
    for name, info in agents_data.items():
        with st.expander(f"Агент: {name}"):
            ...
            role_inputs[name] = st.text_input(
                "Role",
                value=info.get("role", ""),
                key=f"r_{name}",
            )

            goal_inputs[name] = st.text_area(
                "Goal",
                value=info.get("goal", ""),
                key=f"g_{name}",
            )

            backstory_inputs[name] = st.text_area(
                "Backstory",
                value=info.get("backstory", ""),
                key=f"b_{name}",
            )
...
with col2:
    ...
    for name, info in tasks_data.items():
        with st.expander(f"Задача: {name}"):
            ...
            description_inputs[name] = st.text_area(
                "Определение",
                value=info.get("description", ""),
                key=f"d_{name}",
            )

            expected_inputs[name] = st.text_area(
                "Ожидаемый результат",
                value=info.get("expected_output", ""),
                key=f"o_{name}",
            )

            current_agent = info.get("agent")

            index = agent_names.index(current_agent) if current_agent in agent_names else 0

            agent_inputs[name] = st.selectbox(
                "Назначенный агент",
                options=agent_names,
                index=index,
                key=f"agent_{name}",
...
topic = st.text_input(
    "Тема (topic):",
    value="An individualized rescue plan for a struggling student",
)
...
if st.button("Запустить"):
    ...
    try:
        ...
        if os.path.exists(PLAN_PATH):
            ...
            st.subheader("Ответ эдвайзера")
            st.text(plan_content)

        else:
            st.warning(f"Файл не найден: {PLAN_PATH}")

    finally:
        # restore backups
        if os.path.exists(AGENTS_PATH + ".bak"):
            shutil.move(AGENTS_PATH + ".bak", AGENTS_PATH)

        if os.path.exists(TASKS_PATH + ".bak"):
